...
* [函数指针和指针函数](https://www.cnblogs.com/wuyepeng/p/9607580.html)
* [this指针](#this指针)
* [在C++对象内存布局](#在c对象内存布局)
...
* [static关键字作用及其作用域](#static关键字作用及其作用域)
...
* [extern关键字的使用](#extern关键字的使用)
* [C语言中，定义局部变量时如果未初始化，则值是随机的，为什么？](#C语言中，定义局部变量时如果未初始化，则值是随机的，为什么)
...
# this指针
this只能在成员函数中使用。全局函数，静态函数都不能使用this。实际上，成员函数默认第一个参数为 T* const register this 。  
this指针只能在一个类的成员函数中调用，它表示的是当前对象的地址。  

为什么this指针不能在静态函数中使用？  
静态函数同静态变量一样，它不属于具体的哪一个对象，静态函数表示整个类范围意义上的信息，而this指针却实实在在的对应一个对象，所以this指针不能被静态函数使用了。

this指针是什么时候创建的？  
this指针在成员函数的开始执行前构造的，在成员的执行结束后清除。

this指针如何传递给类中函数的？绑定？还是在函数参数的首参数就是this指针，那么this指针又是如何找到类实例后函数的？  
this是通过函数参数的首参数来传递的。this指针是在调用之前生成的。类在实例化时，只分配类中的变量空间，并没有为函数分配空间。this指针指向当前对象的地址，通过this指针调用的函数就是该类对象的函数。
...
### 1.静态全局变量的作用域
在全局变量前加上关键字static，该变量就被定义为一个静态全局变量，静态全局变量存放在全局数据区域，它的作用域是程序的整个运行周期，但是静态全局变量只能在当前文件使用，不能通过使用extern在其他文件使用，而全局变量可以通过extern在其他文件使用。

### 2.静态局部变量
在局部变量前加上static关键字，该变量就被定义成为一个静态局部变量。静态局部变量具有全局寿命，局部可见，只第一次进入函数的时候被初始化，静态局部变量在全局数据区分配内存，如果没有显示初始化，会被程序自动初始化为0。它始终驻留在全局数据区，直到程序运行结束。但其作用域为局部作用域，当定义它的函数或语句块结束时，其作用域随之结束。

### 3.静态函数
在函数的返回类型前加上static关键字，函数即被定义为静态函数。静态函数与普通函数不同，它只能在声明它的文件当中可见，不能被其他文件使用。  
静态函数的好处：1.静态函数不能被其他文件使用。2.其他文件中可以定义相同名字的函数，不会发生冲突。
...
### 5.静态数据成员
静态数据成员，每个类对象都有自己的拷贝。而静态数据成员被当做是类的成员，无论这个类对象被定义了多少个，静态数据成员在程序中只有一份拷贝，有该类所有对象共享，  
静态数据成员存储在全局数据区，静态数据成员定义时要分配内存空间，所以不能在声明中定义。静态数据成员要在类中声明在类外定义。  
静态数据成员和普通数据成员一样遵循public，protected，private访问原则
...
```
这里b.c中定义了一个变量num，如果main.c中想要引用这个变量，那么可以使用extern这个关键字，注意这里能成功引用的原因是，num这个关键字在b.c中是一个全局变量，也就是说只有当一个变量是一个全局变量时，extern变量才会起作用.
...
int a; // 局部变量定义  
a = 15; // 普通的赋值语句  
如果初始化了则相当于给变量指向的内存赋值，相当于给变量一个确定的值。  

# C语言中，定义全局变量时如果未初始化，则值是0，为什么？
全局变量的值是存放在数据区的，数据区内存是干净的不会重复使用，编译时操作系统会为全局变量申请一块干净的内存，使用完后操作系统会自动回收（同时清除该内存上的数据），一块刚进的内存是不存储任何数据的，所以未初始化时值会是0.
